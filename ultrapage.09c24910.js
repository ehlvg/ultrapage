...
class ConfigManager {
    ...
    saveConfig() {
        try {
            localStorage.setItem("startpage-config", JSON.stringify(this.config));
        } catch (e) {
    ...
    async updateWeather() {
        try {
            ...
            if (data.current_weather) {
                ...
                weatherElement.innerHTML = `<i class="${weatherIcon}"></i><span>${temp}\xb0C</span>`;
            }
        } catch (error) {
            console.error("Failed to fetch weather:", error);
            document.getElementById("weather-info").innerHTML = '<i class="ph ph-cloud"></i><span>--\xb0</span>';
        }
    }
    getWeatherIcon(code) {
        const iconMap = {
...
class SearchManager {
    ...
    handleInput(value) {
        if (value.length > 0) this.showSuggestions(value);
    ...
    handleKeydown(e) {
        ...
        switch(e.key){
            case "Enter":
                e.preventDefault();
                if (this.selectedSuggestion >= 0 && suggestions[this.selectedSuggestion]) this.selectSuggestion(suggestions[this.selectedSuggestion]);
                else this.performSearch(this.searchInput.value);
                break;
            case "ArrowDown":
                e.preventDefault();
    ...
    showSuggestions(query) {
        ...
        Object.keys(engines).forEach((key)=>{
            if (key.toLowerCase().includes(query.toLowerCase()) || query.startsWith(key)) suggestions.push({
                type: "engine",
                key: key,
        ...
        if (query.length > 1) suggestions.push({
            type: "search",
            query: query,
    ...
    renderSuggestions(suggestions) {
        ...
        <div class="suggestion px-5 py-3 cursor-pointer transition-all duration-200 flex items-center gap-3 hover:bg-black/5 dark:hover:bg-white/5" data-index="${index}" data-type="${suggestion.type}" data-query="${suggestion.query || suggestion.key}">
    ...
    updateSuggestionSelection() {
        ...
        suggestions.forEach((el, index)=>{
            if (index === this.selectedSuggestion) {
                el.className = "suggestion selected bg-blue-500 text-white px-5 py-3 cursor-pointer transition-all duration-200 flex items-center gap-3";
                el.querySelector("i").style.color = "white";
            } else {
                el.className = "suggestion px-5 py-3 cursor-pointer transition-all duration-200 flex items-center gap-3 hover:bg-black/5 dark:hover:bg-white/5";
    ...
    performSearch(query) {
        ...
        if (bangMatch) {
            ...
            if (engineUrl) {
                if (searchTerm) window.open(engineUrl + encodeURIComponent(searchTerm), "_blank");
                else window.open(engineUrl, "_blank");
            } else // Fallback to DuckDuckGo
            window.open(`https://duckduckgo.com/?q=${encodeURIComponent(trimmedQuery)}`, "_blank");
        } else // Regular search with DuckDuckGo
        window.open(`https://duckduckgo.com/?q=${encodeURIComponent(trimmedQuery)}`, "_blank");
        this.searchInput.value = "";
        this.hideSuggestions();
        this.searchInput.blur();
...
class TabManager {
    ...
    setupTabs() {
        ...
        // Create tabs
        tabsContainer.innerHTML = config.sections.map((section, index)=>`
        <button class="px-5 py-3 rounded-2xl cursor-pointer transition-all duration-300 bg-transparent border-none text-gray-600 dark:text-gray-400 text-sm font-medium whitespace-nowrap flex items-center gap-2 hover:bg-black/5 dark:hover:bg-white/5 hover:text-gray-900 dark:hover:text-white ${index === 0 ? "bg-white/90 dark:bg-gray-800/90 text-gray-900 dark:text-white shadow-lg -translate-y-0.5" : ""}" data-tab="${index}">
            <i class="ph ph-${section.icon}"></i>
            <span>${section.name}</span>
        </button>
    `).join("");
        // Create content
        contentContainer.innerHTML = config.sections.map((section, index)=>`
        <div class="animate-fade-in ${index === 0 ? "block" : "hidden"}" data-content="${index}">
    ...
    `).join("");
        ...
        tabsContainer.addEventListener("click", (e)=>{
            ...
            if (tab) this.switchTab(parseInt(tab.dataset.tab));
        });
    }
    switchTab(index) {
        ...
        tabs.forEach((tab, i)=>{
            if (i === index) {
                tab.className = tab.className.replace(/bg-transparent|bg-white\/90|dark:bg-gray-800\/90|text-gray-600|dark:text-gray-400|text-gray-900|dark:text-white|shadow-lg|-translate-y-0\.5/g, "");
                tab.className += " bg-white/90 dark:bg-gray-800/90 text-gray-900 dark:text-white shadow-lg -translate-y-0.5";
            } else {
                tab.className = tab.className.replace(/bg-white\/90|dark:bg-gray-800\/90|text-gray-900|dark:text-white|shadow-lg|-translate-y-0\.5/g, "");
                tab.className += " bg-transparent text-gray-600 dark:text-gray-400";
            }
        });
    ...
    switchToTab(direction) {
        const config = this.configManager.getConfig();
        const newTab = (this.currentTab + direction + config.sections.length) % config.sections.length;
        this.switchTab(newTab);
    }
    switchToTabNumber(number) {
        const config = this.configManager.getConfig();
        if (number >= 1 && number <= config.sections.length) this.switchTab(number - 1);
    }
}
...
class VimNavigation {
    ...
    setupKeyboardListeners() {
        document.addEventListener("keydown", (e)=>{
            ...
            if (document.activeElement === this.searchManager.searchInput) return;
            switch(e.key){
                case "/":
                    e.preventDefault();
...
function changeGradient(number) {
    ...
    body.className = body.className.replace(/gradient-\d+/, `gradient-${number}`);
    // Save to config
...
function editSection(index) {
    const config = window.configManager.getConfig();
    const section = index >= 0 ? config.sections[index] : {
        name: "",
        icon: "",
        items: []
    };
...
function deleteSection() {
    if (window.currentEditingSection >= 0 && confirm("Are you sure you want to delete this section?")) {
        const config = window.configManager.getConfig();
...
document.getElementById("section-form").addEventListener("submit", (e)=>{
    ...
    if (window.currentEditingSection >= 0) config.sections[window.currentEditingSection] = {
        ...config.sections[window.currentEditingSection],
        ...sectionData
    };
    else config.sections.push(sectionData);
    window.configManager.saveConfig();
    closeSectionEdit();
    renderSectionsList();
    window.tabManager.setupTabs();
...
document.addEventListener("DOMContentLoaded", ()=>{
    ...
    const config = window.configManager.getConfig();
    if (config.selectedGradient) changeGradient(config.selectedGradient);
    // Get user's location
...
//# sourceMappingURL=ultrapage.09c24910.js.map
